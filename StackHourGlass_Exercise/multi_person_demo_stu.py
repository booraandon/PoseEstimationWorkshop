...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_folder", nargs='?', type=str,required=False,default='./example',
    ...
    if args.image:
    ...
    elif "input_folder" in args:
        input_files = sorted(glob.glob(os.path.join(args.input_folder, '*.jpg')), reverse=False)
        ...
        for sFile in input_files:
            cntFiles += 1
            try:
                image = Image.open(sFile).convert('RGB')

                sOutFile = os.path.basename(sFile)
                ...
                r_image = hourglass.detect_image(image)
                r_image.save(os.path.join(args.output, sOutFile))
